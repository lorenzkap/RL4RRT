...
def fit_action_bins(input_amounts, ultrafiltrate_output, n_action_bins=5):
    """
    ...
    ultrafiltrate_cutoffs = [0.0] + np.percentile(ultrafiltrate_output[ultrafiltrate_output > 0], bin_percentiles).tolist()
    vc = np.digitize(ultrafiltrate_output, ultrafiltrate_cutoffs)
    median_ultrafiltrate = [
        np.median(ultrafiltrate_output[vc == bin_num])
        for bin_num in range(1, n_action_bins + 1)
    ]
    ...
    return actions, (median_inputs, median_ultrafiltrate), (input_cutoffs, ultrafiltrate_cutoffs)
    
def transform_actions(input_amounts, ultrafiltrate_output, cutoffs):
    """
    ...
    """
    input_cutoffs, ultrafiltrate_cutoffs = cutoffs
    return (
        len(input_cutoffs) * (np.digitize(input_amounts, input_cutoffs) - 1) +
        (np.digitize(ultrafiltrate_output, ultrafiltrate_cutoffs) - 1)
    )
...
def compute_wis_estimator(sequences, gamma, save_roh=False):
    """
...
def evaluate_policy_wis(metadata, physician_probabilities, model_probabilities, reward_vals, gamma, num_iter, save_roh=False):
    """
