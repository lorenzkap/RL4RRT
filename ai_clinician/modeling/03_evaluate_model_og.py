...
if __name__ == '__main__':
    ...
    actions = transform_actions(
        ...
        MIMICraw[C_ULTRAFILTRATE_OUTPUT],
        action_bins
    )
    ...
    qldata = np.zeros([len(records), 4]) # TODO

    records_no_hidden = records
    ...
    qldata[:, 2] = MIMICraw[C_ULTRAFILTRATE_OUTPUT] - np.array(action_medians[1])[optimal_action[records_no_hidden[C_STATE].to_numpy()] % len(action_bins[1])]
    
    r = pd.DataFrame(qldata)
    # r.columns = ['id','morta','vaso','ivf']
    r.columns = ['id','morta','ultrafiltrate','ivf']
    ...
    d3 = np.array([d.morta['mean'].to_numpy(), d.ultrafiltrate['mean'].to_numpy(), d.ivf['mean'].to_numpy(), d.ultrafiltrate['median'].to_numpy(), d.ivf['median'].to_numpy(), d.ivf['sum'].to_numpy(), groupCount]).T
    ...
    print('Sampling and computing mean mortality for Ultrafiltrate...')
    for rep in tqdm.tqdm(range(nr_reps)):
        vp_mean_sum = []
    ...
    print('Computing std of mortality for Ultrafiltrate...')
    ...
    print('Drawing ultrafiltrate (mortality-dose excess) line chart...')
    # vaso_dose_plot = fig.add_subplot(235)
    ultrafiltrate_dose_plot = fig.add_subplot(235)
    ...
    # vaso_dose_plot.plot(vp_mean-f*vp_sample_std,'b:', linewidth=1)
    h = ultrafiltrate_dose_plot.plot(vp_mean, 'b', linewidth=1)
    ultrafiltrate_dose_plot.plot(vp_mean+f*vp_sample_std,'b:', linewidth=1)
    ultrafiltrate_dose_plot.plot(vp_mean-f*vp_sample_std,'b:', linewidth=1)

    
    # vaso_dose_plot.plot([len(vp_mean)/2, len(vp_mean)/2], [0, 1], 'k:')
    ultrafiltrate_dose_plot.plot([len(vp_mean)/2, len(vp_mean)/2], [0, 1], 'k:')


    
    # vaso_dose_plot.set_xlim([1, len(vp_mean)])
    # vaso_dose_plot.set_ylim([0, 1])
    ultrafiltrate_dose_plot.set_xlim([1, len(vp_mean)])
    ultrafiltrate_dose_plot.set_ylim([0, 1])
    ...
    ultrafiltrate_dose_plot.set_xticks(np.arange(0, 2*len(vp_ticks), 2))
    ultrafiltrate_dose_plot.set_xticklabels(vp_ticks.tolist())
    ...
    for tick in ultrafiltrate_dose_plot.get_xticklabels():
        tick.set_rotation(90)
    ultrafiltrate_dose_plot.set_xlabel('Average dose excess per patient')
    ultrafiltrate_dose_plot.set_ylabel('Mortality')

    # vaso_dose_plot.set_title('Vasopressors')
    ultrafiltrate_dose_plot.set_title('Ultrafiltrate')
    ...
    fig.savefig(os.path.join(fig_path, "ql_wis_histogram.png"))
    # fig.savefig(os.path.join(fig_path, "ql_wis_histogram.pdf"))
    plt.close(fig)
